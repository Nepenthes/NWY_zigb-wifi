...
osThreadId tid_com4DataTransP2_Thread;
osThreadId tid_com5DataTransP3_Thread;
...
osPoolDef (datsZigbTrans_pool, 5, datsAttr_ZigbTrans); // Declare memory pool	//ZIGB数据传输 数据结构池
...
osPoolDef (threadDatsPass_pool, 6, stt_threadDatsPass);  // Declare memory pool
osPoolId  (threadDP_poolAttr_id);                   	 // Memory pool ID
osMessageQDef(MsgBox_threadDP_Z2W, 3, &stt_threadDatsPass);   // Define message queue
osMessageQId  mqID_threadDP_Z2W;
osMessageQDef(MsgBox_threadDP_W2Z, 3, &stt_threadDatsPass);   // Define message queue
...
void USART2_DbugP2_Init(void){
    ...
    Driver_USART2.Send("i'm usart2 for debugP2\r\n", 24);
}
...
void USART3_DataTransP1_Init(void){
    ...
    Driver_USART3.Send("i'm usart3 for dataTransP1\r\n", 28);
}

void USART4_DataTransP2_Init(void){
	
    GPIO_InitTypeDef GPIO_InitStructure;
	
    /*Initialize the USART driver */
    Driver_USART4.Initialize(USART4_callback);
    /*Power up the USART peripheral */
    ...
    Driver_USART4.Send("i'm usart4 for dataTransP2\r\n", 28);
}
...
void USART5_DataTransP3_Init(void){
    ...
    Driver_USART5.Send("i'm usart5 for dataTransP3\r\n", 28);
}
...
/*zigbee设备信息链表节点输出，根据PCB物理地址及扩展地址		！！！谨记使用完取出的节点信息内存后要释放！！！*/ 
nwkStateAttr_Zigb *zigbDev_eptPutout_BYpsy(nwkStateAttr_Zigb *pHead,u8 psyAddr,u8 extAddr,bool method){		//method = 1,源节点地址返回；method = 0,源节点信息映射地址返回，地址操作不会影响源节点
...
/*zigbee设备信息链表节点输出，根据PCB物理地址及扩展地址*/ 
bool zigbDev_eptRemove_BYpsy(nwkStateAttr_Zigb *pHead,u16 psyAddr,u16 extAddr){
...
/*zigbee设备信息链表遍历  遍历 PCB物理地址、扩展地址 将其打包输出*/
u8 ZigBdevDispList(nwkStateAttr_Zigb *pHead,u8 DevInform[]){
	...
	while(Disp->next != NULL){
		...
		DevInform[loop * 2] 	= Disp->psyAddr;
		DevInform[loop * 2 + 1] = Disp->extAddr;
		loop ++;
	}
...
bool ZigB_datsTX(uint8_t  UART_OBJ,
	...
	if(true == responseIF){
		...
		if(TXCMP_FLG && ZigB_datsRX(UART_OBJ,local_datsRX,200)){
...
void ZigB_mainThread(uint8_t UART_OBJ){
	...
	const u16 ZigbDevDetect_Period = 1000;
	u16 ZigbDevDetect_Count = ZigbDevDetect_Period;
	
	u8 loop = 0;
	
	const u8 datsTX_devListLen = 60;			/*测试数据段*/
	u8 datsTX_devList[datsTX_devListLen] = {0};	
	bool TXCMP_FLG = false;
...
//	while(1)osDelay(1000);	/*调试代码段*/
	...
	for(;;){
		...
		/*zigbee在线设备节点检测*/
//		if(ZigbDevDetect_Count < ZigbDevDetect_Period)ZigbDevDetect_Count ++;
//		else{
//		
//			osTimerId tid_ZigbDevDetect;
//			tid_ZigbDevDetect = osTimerCreate(osTimer(TimerZigbDevManage), osTimerOnce, zigbDevList_Head);
//		}
		
		/*WIFI消息数据接收*/
		evt = osMessageGet(mqID_threadDP_W2Z, 5);  // wait for message
		if(evt.status == osEventMessage){
			...
			switch(rptr_W2Z->msgType){
				...
				case conventional:{
					...
					nwkStateAttr_Zigb *infoZigbDevRet_temp = zigbDev_eptPutout_BYpsy(zigbDevList_Head,rptr_W2Z->dats.dats_conv.psyAddr,rptr_W2Z->dats.dats_conv.extAddr,false);
					
					if(NULL != infoZigbDevRet_temp){
					
						memset(dats_kernelTXBUF,0,dats_kernelTXBUFLen * sizeof(u8));
						
						dats_kernelTXBUF[0] = rptr_W2Z->dats.dats_conv.psyAddr;
						dats_kernelTXBUF[1] = rptr_W2Z->dats.dats_conv.extAddr;
						memcpy(&dats_kernelTXBUF[2],rptr_W2Z->dats.dats_conv.dats,rptr_W2Z->dats.dats_conv.datsLen);
						TXCMP_FLG = ZigB_datsTX(UART_OBJ,
												...
												13,
												(u8 *)dats_kernelTXBUF,
												rptr_W2Z->dats.dats_conv.datsLen + 2,	//额外加上pcb物理地址一字节和扩展地址一字节
												true);
		...
		if(true == ZigB_datsRX(UART_OBJ,local_datsRX,50)){
			...
			switch(local_datsRX->datsType){
				...
				case zigbTP_MSGCOMMING:{
					...
					ZigbDevNew_temp = zigbDev_eptPutout_BYnwk(zigbDevList_Head,local_datsRX->datsSTT.stt_MSG.Addr_from,false);
					
					if(NULL == ZigbDevNew_temp){	//设备表未能找到对应网络地址设备则新增节点，否则对应节点设备在线检测周期重置
					
						if(local_datsRX->datsSTT.stt_MSG.Addr_from != 0 && local_datsRX->datsSTT.stt_MSG.datsLen >= 4){	//数据包含两字节指令 和 PCB物理地址及扩展地址，所以数据长度必需大于4，同时节点网络地址不能为0
						
							ZigbDevNew_tempInsert.nwkAddr = local_datsRX->datsSTT.stt_MSG.Addr_from;
							ZigbDevNew_tempInsert.psyAddr = local_datsRX->datsSTT.stt_MSG.dats[2];	//第三字节pcb物理地址
							ZigbDevNew_tempInsert.extAddr = local_datsRX->datsSTT.stt_MSG.dats[3];	//第四字节扩展地址
						...
						switch(cmdLoop){
							...
							case 0:{	//指令0对应操作
								...
								TXCMP_FLG = ZigB_datsTX(UART_OBJ,
														...
														13,
														(u8 *)datsTX_devList,
														devNum * 2 + 1,		//（一字节PCB物理地址 + 一字节扩展地址）*devNum + 长度
														true);
								
								memset(disp,0,dispLen * sizeof(char));
								sprintf(disp,"接收到特殊指令，Zigbee节点设备表数据回发动作已触发\r\n特殊数据远端响应回复是否成功：%d\r\n",TXCMP_FLG);	/*调试代码段*/
								DbugP1TX(disp,strlen((char *)disp));	
								osDelay(20);
							}break;
							...
							case 1:{	//指令1对应操作
								...
								mptr_Z2W->dats.dats_conv.psyAddr = local_datsRX->datsSTT.stt_MSG.dats[2];	//第三字节pcb物理地址
								mptr_Z2W->dats.dats_conv.extAddr = local_datsRX->datsSTT.stt_MSG.dats[3];	//第四字节扩展地址
								memcpy(mptr_Z2W->dats.dats_conv.dats,&(local_datsRX->datsSTT.stt_MSG.dats[4]),local_datsRX->datsSTT.stt_MSG.datsLen - 4);
								mptr_Z2W->dats.dats_conv.datsLen = local_datsRX->datsSTT.stt_MSG.datsLen - 4;
		...
		if(ptr != NULL){
			...
			switch(loop){
				...
				case 0:{
					...
					xorLocation = *(ptr + 7) - '0' + 8;
					if(*(ptr + 9) == WIFI_FRAME_HEAD && *(ptr + xorLocation) == XOR_CHECK((ptr + 10),*(ptr + 7) - '0' - 2)){		//帧头校验及帧尾异或校验
						
						datsRX->linkObj = *(ptr + 5) - '0';
						memcpy(datsRX->CMD,ptr + 11,2);
						memcpy(datsRX->dats,ptr + 13,*(ptr + 10));
						datsRX->datsLen = *(ptr + 10);
...
bool WIFI_datsTX(uint8_t UART_OBJ,
	...
	if(true == responseIF){
		...
		if(TXCMP_FLG && true == WIFI_datsRX(UART_OBJ,local_datsRX,200)){
...
//			CONNECT_HOOK = 0x1f; //连接数目调试值/*调试语句*/
		...
		/*Zigbee消息数据接收*/
		evt = osMessageGet(mqID_threadDP_Z2W, 5);  // wait for message
		if(evt.status == osEventMessage){
			...
			switch(rptr_Z2W->msgType){
				...
				case conventional:{
					...
					if(CONNECT_HOOK){
						...
						memset(dats_kernelTXBUF,0,dats_kernelTXBUFLen * sizeof(u8));
						dats_kernelTXBUF[0] = rptr_Z2W->dats.dats_conv.psyAddr;
						dats_kernelTXBUF[1] = rptr_Z2W->dats.dats_conv.extAddr;
						memcpy(&dats_kernelTXBUF[2],rptr_Z2W->dats.dats_conv.dats,rptr_Z2W->dats.dats_conv.datsLen);
		...
		if(true == WIFI_datsRX(UART_OBJ,local_datsRX,50)){
			...
			switch(local_datsRX->datsType){
					...
					if(cmdLoop < datsTrans_cmdNum){
						...
						switch(cmdLoop){
							...
							case 1:{	//指令1对应操作 常规无线数据传输请求
								...
								mptr_W2Z->dats.dats_conv.psyAddr = local_datsRX->dats[0];
								mptr_W2Z->dats.dats_conv.extAddr = local_datsRX->dats[1];
								memcpy(mptr_W2Z->dats.dats_conv.dats,&local_datsRX->dats[2],local_datsRX->datsLen - 2);
								mptr_W2Z->dats.dats_conv.datsLen = local_datsRX->datsLen - 2;
				...
				case wifiTP_ntCONNECT:{
					...
					snprintf(disp,dispLen,"单元：%d 链接已创建\r\n",local_datsRX->linkObj);
					DbugP1TX(disp,strlen((char *)disp));	
					osDelay(20);
				}break;
				...
				case wifiTP_ntDISCONNECT:{
					...
					snprintf(disp,dispLen,"单元：%d 链接已断开\r\n",local_datsRX->linkObj);
...
void TimerZigbDevManage_Callback(void const *arg){
	...
	while(pHead_listDevInfo->next != NULL){
		...
		if(pHead_listDevInfo->onlineDectect_LCount > 0)pHead_listDevInfo->onlineDectect_LCount --;
			...
			zigbDev_eptRemove_BYnwk((nwkStateAttr_Zigb *)arg,pHead_listDevInfo->nwkAddr);
		}
	}
}
