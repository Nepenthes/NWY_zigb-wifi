...
void TimerBL_Callback(void const *arg){
...
void LCD144Disp_Active(void){
	...
	osTimerId TmrID_tmrBL;
	
	LCD144_Init();

	TmrID_tmrBL = osTimerCreate(osTimer(TimerBL), osTimerPeriodic, NULL);
	osTimerStart(TmrID_tmrBL, 11UL);
	
	tid_LCD144Disp_Thread = osThreadCreate(osThread(LCD144Test_Thread),NULL);
}
